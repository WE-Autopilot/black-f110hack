...
from lidar_utils import make_360_scan, lidar_to_image
...
def main():
    ...
    scans = np.array(obs['scans']).squeeze()  # Convert to 1D array
    env.render()

    laptime = 0.0
    start = time.time()
    last_update_time = start  # Initialize the timer
    ...
    while not done:
        ...
        full_scan = make_360_scan(scans)
        
        # Convert to image
        lidar_image = lidar_to_image(full_scan)
        
        # Visualize
        current_time = time.time()
        if current_time - last_update_time >= 0.01:  # Update every 10ms
